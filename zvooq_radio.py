...
class StationNode:
    ...
    def getBackground(self):
        return self.source.station.icon.background_color
...
class Radio:
    def __init__(self, client, station_id, station_from, high_res = False):
    ...
    def __start_radio(self, callback):
        ...
        current_url = get_track_url(track=current, high_res=self.high_res)

        next_track = self.tracks[1]
        next_track.get_download_info()
        next_track_url = get_track_url(track=next_track, high_res=self.high_res)
